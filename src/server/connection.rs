...
impl Connection {
    ...
    //                 // Chercher la fin des headers
    //                 if let Some(headers_end) = find_headers_end(&self.buffer) {
    //                     // On a les headers complets, on peut commencer le parsing
    //                     let mut buffer_complet = false;
                        
    //                     // Vérifier si on a tout le body avec Content-Length
    //                     if let Ok(request_str) = String::from_utf8(self.buffer[..headers_end].to_vec()) {
    //                         if let Some(content_length) = get_content_length(&request_str) {
    //                             if self.buffer.len() >= headers_end + content_length {
    //                                 buffer_complet = true;
    //                             }
    //                         } else {
    //                             // Pas de Content-Length, on considère qu'on a tout
    //                             buffer_complet = true;
    //                         }
    //                     }
    
    //                     if buffer_complet {
    //                         // On a toutes les données, on peut parser
    //                         match parse_request(&self.buffer) {
    //                             Some(request) => {
    //                                 // Vider le buffer après un parsing réussi
    //                                 let request_len = self.buffer.len();
    //                                 self.buffer.clear();
    //                                 return Ok(request);
    //                             }
    //                             None => continue // Continuer à lire si le parsing échoue
    //                         }
    //                     }
    //                 }
